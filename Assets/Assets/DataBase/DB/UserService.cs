...
{
    private static string UserPath = @"E:\DataBase\Users.txt";
    private static string GamesPath = @"E:\DataBase\Games.txt";
        ...
        try
        {
            using (StreamReader readtext = File.OpenText(GamesPath))
            {
                ...
                while ((s = readtext.ReadLine()) != null)
                {
                    ...
                    Game addGame = gf.CreateGame(gameType, DBContext_.Users.Find(p1 => p1.ID == player1), DBContext_.Users.Find(p2 => p2.ID == player2), rating);
